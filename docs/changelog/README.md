...
* **Custom CSP must allow the basemap** — maps fetch their style document, glyphs, sprite and vector tiles over `fetch`/XHR, so `connect-src` governs them. The shipped policy now allows `https://basemaps.cartocdn.com`, `https://*.basemaps.cartocdn.com` and `https://tile.openstreetmap.org`; if you override the policy at a proxy or ingress, mirror those three or every map renders its legend over blank white. See [Security](../features/security.md#basemap-origins). ([a4b738a7](https://github.com/depictio/depictio/commit/a4b738a7))
* **Text tiles centre vertically by default** — `vertical_alignment` defaults to `center`, so a one-line heading in a tall tile no longer pins to the top edge. Set `vertical_alignment: top` to keep the previous rendering; the existing `alignment` field is now labelled *horizontal* in the builder. See [Components](../features/components.md#text-components). ([c647b526](https://github.com/depictio/depictio/commit/c647b526), [0366d5fc](https://github.com/depictio/depictio/commit/0366d5fc))
* **Helm `enableServiceLinks: false`** — the chart now disables Kubernetes' legacy Docker-link service variables on every pod, which Kubernetes itself leaves on. On busy namespaces those variables overflowed the viewer's argument limit. See [Kubernetes installation](../installation/kubernetes.md#service-link-environment-variables). ([99061052](https://github.com/depictio/depictio/commit/99061052))

### **✨ New Features**

* **Dashboard sections** — group the grid and the filter panel into named, foldable sections via `grid_sections` and `filter_sections`. A folded section fetches nothing until you open it, while its header still reports its numbers or a component count. See [YAML Sync](../features/yaml-sync.md#dashboard-sections). ([071b7e84](https://github.com/depictio/depictio/commit/071b7e84), [da1049f8](https://github.com/depictio/depictio/commit/da1049f8), [f9a719dd](https://github.com/depictio/depictio/commit/f9a719dd))
* **Dashboard-wide map panel** — a map authored with `placement: floating` leaves the grid and follows you across every tab of a dashboard, as a draggable card, as a dock under the filter panel, or hidden behind a header control that keeps showing how many values it filters on. You can also open the rows the map is drawing and tick them to select. See [Components](../features/components.md#dashboard-wide-map-panel). ([0c66045b](https://github.com/depictio/depictio/commit/0c66045b), [bfa76ff5](https://github.com/depictio/depictio/commit/bfa76ff5), [9a6e0935](https://github.com/depictio/depictio/commit/9a6e0935), [7e18d931](https://github.com/depictio/depictio/commit/7e18d931), [9909825a](https://github.com/depictio/depictio/commit/9909825a))
* **Ten new card layouts** — `secondary_layout` gains `histogram`, `threshold`, `completeness`, `uniqueness`, `attrition`, `trend`, `gauge`, `grid`, `composition` and `donut`, covering distribution shape, QC cut-offs, missingness, retention across pipeline stages and composition. See [Components](../features/components.md#secondary-layout-modes). ([93488c7e](https://github.com/depictio/depictio/commit/93488c7e), [532df542](https://github.com/depictio/depictio/commit/532df542), [be63bb45](https://github.com/depictio/depictio/commit/be63bb45), [beaa16c7](https://github.com/depictio/depictio/commit/beaa16c7))
* **Component inspector** — an opt-in docked panel that replaces the per-component popovers for advanced-visualisation controls, notes and metadata. Off by default; enable with `DEPICTIO_VIEWER_INSPECTOR_ENABLED`. ([071b7e84](https://github.com/depictio/depictio/commit/071b7e84))

### **🚀 Improvements**

* **Filter order survives into view mode** — the editor and the viewer now draw the same panel, so the order you drag filters into is the order everyone sees. It collapses to a 44px rail that keeps the active-filter count on screen, its width is draggable and remembered per dashboard, and it becomes a drawer on narrow screens. Active filters are an aligned, foldable list with a per-row clear rather than a wrapping badge strip. ([da1049f8](https://github.com/depictio/depictio/commit/da1049f8), [dd4ef09f](https://github.com/depictio/depictio/commit/dd4ef09f), [232ba576](https://github.com/depictio/depictio/commit/232ba576))
* **Real creation dates** — dashboards and projects list a **Created** date distinct from **Modified**, stored in UTC and rendered in your own timezone with the original value in the tooltip. Timestamp columns sort by instant rather than by string. ([1e4573c9](https://github.com/depictio/depictio/commit/1e4573c9))
* **Table controls on both listings** — a per-table column picker whose choice persists, compact mode on the projects table, and workflow badges carrying each system's own logo and brand colour. ([1e4573c9](https://github.com/depictio/depictio/commit/1e4573c9), [96dae01f](https://github.com/depictio/depictio/commit/96dae01f), [41591e6a](https://github.com/depictio/depictio/commit/41591e6a), [c600b2a0](https://github.com/depictio/depictio/commit/c600b2a0))
* **Dependency refresh** — runtime and static-tooling groups bumped, with beanie and redis held at their pinned versions for reasons recorded in the pin comments. ([1281ed94](https://github.com/depictio/depictio/commit/1281ed94), [07db0f2d](https://github.com/depictio/depictio/commit/07db0f2d))

### **🐛 Bug Fixes**

* **Sessions last the whole page session** — a tab left open past the access token's lifetime broke silently: dashboard autosaves stopped persisting, the admin **Log & Task** panel 401'd on every poll with "Invalid token", and in single-user mode a signed-in user's own projects simply stopped appearing. Every API call now refreshes, a background keep-alive runs often enough that an idle tab cannot lapse between ticks, and logging out stays logged out. ([f1f9db29](https://github.com/depictio/depictio/commit/f1f9db29), [08219a5e](https://github.com/depictio/depictio/commit/08219a5e), [a246e0c4](https://github.com/depictio/depictio/commit/a246e0c4), [db2b99c9](https://github.com/depictio/depictio/commit/db2b99c9))
* **Cards under filters** — `skewness`, `kurtosis` and `percentile` blanked to "—" as soon as any filter was applied, a card's own `filter_expr` was validated but never applied so conditional cards showed unfiltered numbers, and a card built with a distinct count on a numeric column failed YAML export. The builder preview also showed fabricated buckets instead of the real data. ([532df542](https://github.com/depictio/depictio/commit/532df542), [be63bb45](https://github.com/depictio/depictio/commit/be63bb45), [beaa16c7](https://github.com/depictio/depictio/commit/beaa16c7))
* **Figures** — an axis now claims the margin its labels need, so long rotated category names are no longer cut off under the axis title, and aggregated box plots and overlay histograms honour the author's `color_discrete_map`, `color_discrete_sequence` and `opacity` instead of falling back to Plotly's palette. ([31ee05f8](https://github.com/depictio/depictio/commit/31ee05f8), [7da123f0](https://github.com/depictio/depictio/commit/7da123f0))
* **Dashboard saves** — a save no longer fails on MongoDB's immutable `_id` or blanks the thumbnail cache-buster, which had been letting listings re-serve stale images, and seeded projects with hand-written ObjectIds no longer claim a creation date years in the future. ([b90d171c](https://github.com/depictio/depictio/commit/b90d171c), [69318520](https://github.com/depictio/depictio/commit/69318520))
* **Load all rows** — offered only when a table spans more than five pages, rather than on every table wider than a single page. ([48b54106](https://github.com/depictio/depictio/commit/48b54106))
...
* **Date columns broke ingestion** — a Date or Time column's min/max reached MongoDB as a `datetime.date`, which BSON cannot encode, so the Delta upsert answered 500, no data-collection document was written, the freshly written S3 prefix was reaped as an orphan, and every component bound to that collection answered 404. The ingest logged it as a skipped optional collection and still reported success. Only reproduced where collections are re-ingested from scratch. ([21fa67ab](https://github.com/depictio/depictio/commit/21fa67ab))
* **Seeded default thumbnails** — the 64 thumbnails a fresh deployment seeds had been captured against a stack whose data collections were never ingested and committed as a side effect of an unrelated change, so since July 21 new deployments showed error banners and "—" in place of their dashboards. Restored, with a pre-commit hook that now refuses to stage them unless the regeneration is deliberate. ([275be00e](https://github.com/depictio/depictio/commit/275be00e), [164fbb3c](https://github.com/depictio/depictio/commit/164fbb3c))

---

## **v1.3.1 Beta Releases**

!!! warning "Pre-release builds leading up to v1.3.1"
    Beta images are published for testing ahead of the stable release. Use the stable `v1.3.1` image for production.
