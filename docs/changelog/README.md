...
* **Custom CSP must allow the basemap** — `connect-src` now needs `https://basemaps.cartocdn.com`, `https://*.basemaps.cartocdn.com` and `https://tile.openstreetmap.org`. Override the policy at a proxy or ingress without them and every map renders blank. See [Security](../features/security.md#basemap-origins) ([#951](https://github.com/depictio/depictio/pull/951)).
* **Text tiles centre vertically** — `vertical_alignment` now defaults to `center`; set it to `top` to keep the previous rendering. See [Components](../features/components.md#text-components) ([#951](https://github.com/depictio/depictio/pull/951), [#954](https://github.com/depictio/depictio/pull/954)).
* **Helm `enableServiceLinks: false`** — the chart now disables Kubernetes' legacy service variables on every pod, the opposite of Kubernetes' own default. See [Kubernetes](../installation/kubernetes.md#service-link-environment-variables) ([#946](https://github.com/depictio/depictio/pull/946)).

### **✨ New Features**

* **Dashboard sections** — group the grid and the filter panel into named, foldable sections with `grid_sections` and `filter_sections`; a folded section fetches nothing until you open it. See [YAML Sync](../features/yaml-sync.md#dashboard-sections) ([#954](https://github.com/depictio/depictio/pull/954)).
* **Dashboard-wide map panel** — `placement: floating` lifts a map out of the grid to follow you across every tab, floating, docked or hidden, with a table of the rows behind it. See [Components](../features/components.md#dashboard-wide-map-panel) ([#951](https://github.com/depictio/depictio/pull/951)).
* **Ten new card layouts** — `secondary_layout` gains `histogram`, `threshold`, `completeness`, `uniqueness`, `attrition`, `trend`, `gauge`, `grid`, `composition` and `donut`. See [Components](../features/components.md#secondary-layout-modes) ([#951](https://github.com/depictio/depictio/pull/951)).
* **Component inspector** — an opt-in docked panel replacing the per-component popovers. Off by default; enable with `DEPICTIO_VIEWER_INSPECTOR_ENABLED` ([#954](https://github.com/depictio/depictio/pull/954)).

### **🚀 Improvements**

* **Filter panel** — editor and viewer draw the same panel, so authored order is what a viewer sees. It collapses to a rail that keeps the active-filter count, its width is remembered per dashboard, and active filters are a foldable list with a per-row clear ([#954](https://github.com/depictio/depictio/pull/954)).
* **Real creation dates** — dashboards and projects list a **Created** date distinct from **Modified**, rendered in your own timezone and sorted by instant rather than by string ([#951](https://github.com/depictio/depictio/pull/951)).
* **Table controls on both listings** — a column picker whose choice persists, compact mode on the projects table, and workflow badges carrying each system's own logo and colour ([#951](https://github.com/depictio/depictio/pull/951)).
* **Dependency refresh** — runtime and static-tooling groups bumped ([#947](https://github.com/depictio/depictio/pull/947), [#942](https://github.com/depictio/depictio/pull/942)).

### **🐛 Bug Fixes**

* **Sessions last the whole page session** — a tab left open past the token's lifetime stopped persisting autosaves, 401'd the admin **Log & Task** panel, and hid a user's own projects in single-user mode. Every API call now refreshes ([#940](https://github.com/depictio/depictio/pull/940), [#951](https://github.com/depictio/depictio/pull/951)).
* **Cards under filters** — `skewness`, `kurtosis` and `percentile` no longer blank once a filter is applied, a card's own `filter_expr` is applied rather than only validated, and a distinct count on a numeric column exports ([#951](https://github.com/depictio/depictio/pull/951)).
* **Figures and tables** — axes claim the margin their rotated labels need, aggregated box plots and overlay histograms honour the author's colours, and *Load all rows* is offered only past five pages ([#954](https://github.com/depictio/depictio/pull/954)).
* **Dashboard saves** — no longer fail on MongoDB's immutable `_id` or blank the thumbnail cache-buster, and seeded projects stop claiming a creation date years in the future ([#951](https://github.com/depictio/depictio/pull/951)).
