...
### Managing the report set (React Beta)

From the React viewer (`/dashboards-beta`), MultiQC Data Collections support a full lifecycle without leaving the UI:
...
### Marking a Data Collection as Map-capable

Coordinates can be declared at the **DC level** (recommended) or per-figure. Declaring at the DC level lets every Map figure on that DC inherit the same coordinates without re-specifying `lat_column`/`lon_column`.
...
- **In YAML**: use `DCTableCoordinatesConfig` on the DC's config block (see [YAML examples](../usage/projects/yaml-examples.md)).
